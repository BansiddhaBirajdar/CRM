            ...
            <div class="card">
              <div class="card-header">
                <button class="btn btn-primary btn-md " id="add" data-toggle="modal" data-target="#modal-default">
                  <i class="nav-icon fas fa-plus"></i> Add Issue</button>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td> 
                        ...
                        <i class="fas fa-pencil-alt editfrom" data-status='<?php echo $record->status; ?>' data-issue='<?php echo $record->issue; ?>' data-id='<?php echo $record->id; ?>'> Edit</i>
                          </span>
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <a class="btn btn-danger btn-sm " href="<?php echo site_url('Setup/deleteIssue/').$record->id; ?>".><i class="fas fa-trash" ></i> Delete</a>
                    </tr>
                    <?php }} ?>
                   </tbody>
                </table>
            ...
            <div class="modal-body">
              <div class="card card-primary">
                <form role="form" method="post" id="upload_form"  enctype="multipart/form-data">
                  <div class="card-header bg-primary titleadd">Add Issue</div>
                  <div class="card-header bg-primary titleupdate" style="display: none">Update Issue</div>
                  <div class="card-body">
                    <input type="text" name="id" id="id" hidden>
                    <div class="row">
                      <div class="col-sm-12">
                        <div class="form-group">
                          <label for="exampleInputName1">Issue</label>
...
<script>  
 $(document).ready(function(){  
  ...
  <?php if($this->session->flashdata('delete')){ ?>
      toastr.error('  Successfully Issue Delete .  ')    
  <?php } ?>
  <?php if($this->session->flashdata('insert')){ ?>
    toastr.success(' Successfully Issue Add  ')
  <?php } ?>
  <?php if($this->session->flashdata('update')){ ?>
    toastr.success(' Successfully Issue Update ')
  <?php } ?>
...
$('#add').click(function(e){
  ...
  $('#upload_form').on('submit', function(e){  
      ...
      if(id=='')
      {
        ...
        $("form")[0].reset();
        $.ajax({  
          url:"<?php echo base_url(); ?>Setup/insertIssue",   
          type: 'post',
          data: {issue:issue,status:status},  
        ...
        $("form")[0].reset();
        $.ajax({  
          url:"<?php echo base_url(); ?>Setup/updateIssue",   
          type: 'post',
          data: {id:id,issue:issue,status:status},  
          success:function(data)  
          {  
              // alert(data);
                window.location.href=data;
